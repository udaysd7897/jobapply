...
4. **Escalation triggers**: both hard errors (the agent gets stuck) and low-confidence answers (from the semantic search over job-specific questions) escalate to a human via the same channel. The exact channel (WhatsApp or otherwise) is a technical decision, not fixed by this requirement.
5. **Free-text answers**: generated using the semantic search + JD-reframing approach above. If confidence is low, escalate rather than guess — this bounds the truthfulness risk without a separate honesty rule.
